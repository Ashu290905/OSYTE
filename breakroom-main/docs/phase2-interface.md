...
| `reconciliation_engine/pipeline.py` (check 6) | Mihir | `reconciliation_api/run_reconciliation.py` (Mihir, Phase 3) |
| `ai_inference/schema_detector.py` | Aarohi | `ai_inference/analyzer.py` (Aarohi, Phase 4) |

**Agree by start of Day 5:**
- `FilterResult` and `DuplicateCheckResult` dataclasses — Mihir's `pipeline.py` builds with these; agree before integration
- `Check6Result` dataclass — Mihir's `run_reconciliation.py` unpacks this; agree before Phase 3
- `normalise_record()` + `apply_filter_rules()` signatures — Mihir calls both in `pipeline.py`
- `NormalizationError` exception — add to `models.py` now (missed at Phase 1 lock)
...
# Case-sensitive lookup of value in mappings[i].from_value.
# Returns matching to_value, or original value if no match (soft failure).
# Note: TXT-01 (uppercase) should be applied before TXT-04 so mappings use uppercase keys.
...
# Ashutosh: reconciliation_engine/tolerance.py
...
# Mihir: reconciliation_engine/pipeline.py (check 6)
run_check6(
...
# Aarohi: ai_inference/schema_detector.py
...
def apply_txt04(
    ...
    # Caller applies TXT-01 before TXT-04 if case normalisation is needed
```
...
def check_duplicates_source(
    ...
    # Build composite key from each record using the normalised field values
    # Group records by their composite key string
